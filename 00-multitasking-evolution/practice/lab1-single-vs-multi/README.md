...
- ใน Single Task System การตอบสนองต่อการกดปุ่ม ล่าช้า มาก เพราะระบบจะตรวจสอบปุ่มหลังจากทำงานครบทุกงานในลูป เช่น การอ่านเซนเซอร์ ประมวลผล  ควบคุม LED  แล้วจึงเช็คปุ่ม

- ใน Multitasking System การตอบสนอง รวดเร็วขึ้นมาก เพราะมี emergency_task ที่รันอยู่ตลอดและมี priority สูงสุด ทำให้สามารถตอบสนองทันทีที่กดปุ่ม แทบไม่หน่วงเลย

2. ใน Single Task System งานไหนที่ทำให้การตอบสนองล่าช้า?
- งานที่ทำให้ล่าช้าที่สุดคือ Task 2: Process data
เพราะมีการคำนวณในลูป for (int i = 0; i < 1000000; i++) ซึ่งกินเวลา CPU นานมาก
ทำให้โค้ดส่วนเช็คปุ่มถูกหน่วงออกไปจนกว่าจะประมวลผลเสร็จ นอกจากนี้ งาน Blink LED และ Actuator Control ก็มี vTaskDelay() ที่ทำให้ต้องรออีกก่อนจะเช็คปุ่ม

3. ข้อดีของ Multitasking System ที่สังเกตได้คืออะไร?
- สามารถทำหลายงานได้พร้อมกัน 

- การตอบสนองต่อเหตุการณ์ทันที โดยเฉพาะงานที่มี priority สูง 

- การทำงานของแต่ละงานเป็นอิสระ ไม่ต้องรอให้ลูปของงานอื่นเสร็จก่อน

- ทำให้โค้ดอ่านง่ายและเข้าใจง่ายกว่า เพราะแยกเป็นแต่ละ task ตามหน้าที่

4. มีข้อเสียของ Multitasking System ที่สังเกตได้หรือไม่?
- การจัดการระบบมีความซับซ้อนมากขึ้น ต้องตั้ง priority, stack size และระวังการชนกันของทรัพยากร

- หากจัด priority ไม่ดี อาจเกิด Starvation บาง task ไม่ได้ CPU

- ใช้หน่วยความจำมากกว่า เพราะแต่ละ task ต้องมี stack แยกกัน

- การดีบัก (debugging) ยากกว่า Single Task เพราะงานหลายอย่างเกิดพร้อมกัน
