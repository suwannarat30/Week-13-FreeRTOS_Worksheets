...
|-------|-------------|-----------------|--------------|----------------|------------|
| 1 (Normal) | 15|15 |5 | 3|100 |
| 2 (Multiple Give) |25 | 18| 5|2 | 72|
| 3 (Short Timeout) | 12| 9|4 |1 |75 |

### คำถามสำหรับการทดลоง
1. เมื่อ give semaphore หลายครั้งติดต่อกัน จะเกิดอะไรขึ้น?
- เนื่องจาก Binary Semaphore มีค่าได้เพียง 0 หรือ 1 การ give ซ้ำหลายครั้งจะไม่สะสมค่าเพิ่ม ค่าของ semaphore จะคงเป็น 1 (available) จนกว่าจะถูก take โดย task อื่น ดังนั้น “Multiple Give” จะไม่ทำให้มี event สะสม แต่จะส่งผลเพียงครั้งเดียว

2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?
- ไม่ได้ครับ ต้องใช้ฟังก์ชันที่ออกแบบมาสำหรับ ISR เท่านั้นคือ xSemaphoreGiveFromISR() เพราะมีการจัดการ context switch ที่ปลอดภัยใน interrupt context ถ้าใช้ xSemaphoreGive() ภายใน ISR จะเกิด error หรือพฤติกรรมไม่คาดคิด

3. Binary Semaphore แตกต่างจาก Queue อย่างไร?
- Binary Semaphore ใช้สำหรับ การ Synchronization หรือแจ้งเหตุการณ์ ระหว่าง Task หรือจาก ISR ไปยัง Task โดยมีค่าได้เพียง 0 หรือ 1 เท่านั้น ทำหน้าที่แค่เป็นสัญญาณว่า “เหตุการณ์เกิดขึ้นแล้ว” ไม่เก็บข้อมูลใดๆ และเหมาะกับกรณีที่ต้องการแจ้ง event แบบทันที เช่น การกดปุ่ม หรือการเกิด timer interrupt

- Queue ใช้สำหรับ การสื่อสารข้อมูล (Data Communication) ระหว่าง Task สามารถกำหนดขนาด buffer ได้ตามต้องการ และเก็บ payload ของข้อมูล เช่น ค่า sensor readings หรือ message ระหว่าง Task จึงเหมาะสำหรับการส่งข้อมูลต่อเนื่องหรือหลายค่า ไม่ใช่แค่สัญญาณ
