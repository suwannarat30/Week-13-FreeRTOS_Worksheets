...
- จะเกิด Resource Contention บาง Producers ต้องรอจนกว่าทรัพยากรจะว่าง  Wait Time เพิ่มขึ้น Success Rate ลดลงเพราะบางครั้งรอ timeout เกิดขึ้น

2. Load Generator มีผลต่อ Success Rate อย่างไร?
- Load Generator ทำให้ทรัพยากรถูกจับใช้งานชั่วคราว (hold) ทำให้ Producers ปกติบางตัวต้องรอนานขึ้น → เพิ่ม failed acquisitions → Success Rate ลดลงชั่วคราว เมื่อ load burst ผ่านไป Success Rate จะกลับเข้าสู่ปกติ

3. Counting Semaphore จัดการ Resource Pool อย่างไร?
- Semaphore count = จำนวน resource ว่าง
- xSemaphoreTake() → ลด count ถ้ามี resource ว่าง → Producer ได้ใช้ resource
- xSemaphoreGive() → เพิ่ม count → resource กลับว่าง
- ทำให้ จำนวน Producers ที่ใช้ resource พร้อมกันไม่เกินจำนวนที่กำหนด

สร้างระบบ Rate Limiting / Fair Access ให้ resource pool โดยอัตโนมัติ
## 📋 สรุปผลการทดลอง
