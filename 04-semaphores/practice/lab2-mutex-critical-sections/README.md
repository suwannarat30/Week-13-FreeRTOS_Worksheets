...
|-------|------------|--------|-----------|-------------|---------|
| 1 (With Mutex) |50 | 0| 0| 100| |
| 2 (No Mutex) |50 | 0| 15|100 | |
| 3 (Changed Priority) | 48|2 | 0| 96| |

### คำถามสำหรับการทดลอง
1. เมื่อไม่ใช้ Mutex จะเกิด data corruption หรือไม่?
- ใช่ การเข้าถึง shared resource พร้อมกันจากหลาย task ทำให้เกิด Race Condition และข้อมูลถูกแก้ไขไม่ถูกต้อง (data corruption) เช่น counter ไม่ตรงหรือ buffer ถูก overwrite

2. Priority Inheritance ทำงานอย่างไร?
- เมื่อ task ความสำคัญสูงต้องการ mutex ที่ task ความสำคัญต่ำถืออยู่ ระบบจะเพิ่ม priority ของ task ต่ำชั่วคราวเพื่อให้ task ปล่อย mutex เร็วขึ้น ป้องกัน Priority Inversion

3. Task priority มีผลต่อการเข้าถึง shared resource อย่างไร?
- Task ที่มี priority สูงจะมีโอกาสเข้าถึง critical section ก่อน task ที่ priority ต่ำ หากไม่มี priority inheritance, task ต่ำที่ถือ mutex อาจถูก task ความสำคัญปานกลางขัดขวาง ทำให้ priority inversion เกิดขึ้น
