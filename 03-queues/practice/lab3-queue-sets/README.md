...
|-------|--------|------|---------|-------|-------|---------|
| 1 (ปกติ) |22 |10 |37 |35 |5 |72 |
| 2 (ไม่มี Sensor) |0 |12 |37 |5 |54 | |
| 3 (Network เร็ว) |25 |11 |75 |6 |117 | |

### คำถามสำหรับการทดลอง
1. Processor Task รู้ได้อย่างไรว่าข้อมูลมาจาก Queue ไหน?
-  ฟังก์ชัน xQueueSelectFromSet() มันจะ “บอกกลับ” ว่า Queue ไหนมีข้อมูลเข้ามา เช่น ถ้ามันคืนค่าเป็น xSensorQueue ก็แปลว่าข้อมูลมาจาก Sensor ถ้าเป็น xUserQueue ก็แปลว่ามาจาก User

2. เมื่อหลาย Queue มีข้อมูลพร้อมกัน เลือกประมวลผลอันไหนก่อน?
- ระบบจะประมวลผล อันที่ FreeRTOS ตรวจพบก่อนในลำดับการสแกนภายใน Queue Set ซึ่งโดยทั่วไปลำดับจะไม่คงที่เสมอ ขึ้นอยู่กับช่วงเวลาที่ Task อื่นส่งข้อมูลเข้ามา ดังนั้นผลคือ คิวไหนมาก่อนก็ถูกประมวลผลก่อนแต่ถ้าต้องการควบคุมลำดับจริงๆ ต้องเพิ่ม logic เรื่อง priority handling เอง

3. Queue Sets ช่วยประหยัด CPU อย่างไร?
- เพราะแทนที่ Task จะต้อง xQueueReceive() ทีละคิววนไปเรื่อย ๆ (polling) มันใช้ xQueueSelectFromSet() แค่คำสั่งเดียวรอทุก Queue พร้อมกันได้เลย CPU จึงไม่ต้องตื่นมาดูหลายรอบ ลด context switch และ idle time ทำให้ระบบประหยัดพลังงานและมีประสิทธิภาพสูงขึ้น โดยเฉพาะในระบบ event-driven
